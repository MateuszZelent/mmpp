...
def debug_smart_legend():
    ...
    base_path = "/mnt/storage_2/scratch/pl0095-01/zelent/mannga/bowtie/mateusz/sinc/solver_test"
    ...
    if len(results) > 0:
        ...
        if len(results) >= 2:
            ...
            different_attrs = []
            common_keys = set(result1.attributes.keys()) & set(result2.attributes.keys())
        ...
        varying_params = plotter._get_varying_parameters(results[:2] if len(results) > 1 else results)
