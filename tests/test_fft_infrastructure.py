...
def test_zarr_save_structure():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        spectrum = np.random.random((n_freq, n_comp)) + 1j * np.random.random((n_freq, n_comp))
...
def test_parameter_verification():
    ...
    def compare_parameters(p1, p2):
        ...
        fft_params = ['window_function', 'filter_type', 'fft_engine', 'zero_padding', 'nfft']
        metadata_params = ['z_layer', 'source_dataset']
        
        for key in fft_params:
            if p1.get(key) != p2.get(key):
                print(f"  - FFT parameter difference: {key} = {p1.get(key)} vs {p2.get(key)}")
                return False
        
        for key in metadata_params:
            if p1.get(key) != p2.get(key):
                print(f"  - Metadata parameter difference: {key} = {p1.get(key)} vs {p2.get(key)}")
                return False
