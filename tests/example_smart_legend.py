...
try:
    ...
    for i, result in enumerate(results):
        print(f"  Dataset {i+1}: solver={result.solver}, f0={result.f0:.1e}, "
    ...
    for result in results:
        for attr in dir(result):
            if (not attr.startswith('_') and 
                attr not in ['path', 'attributes'] and
                not callable(getattr(result, attr, None))):
                try:
    ...
    print(f"   results.matplotlib.plot(x_series='t', y_series='m_z11', average=(1,2,3), comp='z')")
    print(f"   # Legend will automatically show only: f0, maxerr, Nx, dt (parameters that vary)")
    print(f"   # Hidden from legend: solver, amp_values, Ny, Nz, PBCx, PBCy, PBCz (constant parameters)")
