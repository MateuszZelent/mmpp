...
try:
    ...
    results = [
        MockResult(solver=1, f0=1e9, maxerr=1e-6, Nx=64, Ny=64, Nz=1, constant_param=100),
        MockResult(solver=2, f0=1e9, maxerr=1e-6, Nx=128, Ny=64, Nz=1, constant_param=100),
        MockResult(solver=1, f0=2e9, maxerr=1e-7, Nx=64, Ny=128, Nz=1, constant_param=100),
    ]
    ...
    if 'constant_param=100' not in str([plotter._format_result_label(r, varying_params) for r in results]):
    ...
    formatted_label = plotter._format_result_label(test_result, ['solver', 'f0', 'maxerr', 'Nx'])
