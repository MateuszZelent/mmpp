...
def test_fmr_modes():
    ...
    try:
        ...
        from mmpp.fft.modes import FMRModeAnalyzer, ModeVisualizationConfig, FFTModeInterface
...
def test_mode_computation():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mode2_spatial = np.sin(2*np.pi*X/x[-1]) * np.cos(2*np.pi*Y/y[-1])
            ...
            analyzer = FMRModeAnalyzer(zarr_path, dataset_name="m", config=config, debug=True)
            ...
            for peak in peaks[:5]:  # Show first 5 peaks
                print(f"   Peak at {peak.freq:.2f} GHz (amplitude: {peak.amplitude:.3f})")
