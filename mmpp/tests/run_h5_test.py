...
with h5py.File(m_dir / "m.h5", "w") as f:
    for s in range(n_steps):
        f.create_dataset(
            str(s), data=rng.standard_normal((nz, ny, nx, ncomp)).astype(np.float32)
        )
    f.create_dataset("t", data=np.linspace(0, 1e-9, n_steps))

table_dir = zarr_dir / "table"
table_dir.mkdir()
...
all_frames = [
    rng2.standard_normal((nz, ny, nx, ncomp)).astype(np.float32) for _ in range(n_steps)
]
