...
class SolitonInterface:
    ...
    def _repr_html_(self) -> str:
        ...
        slice_label = str(self._slice_info) if self._slice_info is not None else "full"
        ...
        namespaces = [
            (
                ".vortex",
                "Vortex dynamics: topology, core, trajectory, spectrum, modes, nonlinear, events",
            ),
            (
                ".skyrmion",
                "Skyrmion topology, centre, radial profile, and size fitting",
            ),
        ]
        ns_rows = "".join(
        ...
        workflow = [
            (
                "1. Topology",
                "vortex.topology.detect()",
                "polarity, chirality, winding number",
            ),
            (
                "2. Track core",
                "vortex.core.track()",
                "auto/table/Gaussian core tracking",
            ),
        ...
        wf_section = (
            f"<div style='{_HELPER_SECTION_CHROME}'>"
            "<b style='color:#94a3b8;'>Vortex Workflow</b><br>"
        ...
        skyrmion_workflow = [
            (
                "1. Topology",
                "skyrmion.detect()",
                "charge Q, centre, polarity, confidence, flags",
            ),
            (
                "2. Measure R50",
                "skyrmion.measure_size()",
                "model-independent contrast crossings",
            ),
            (
                "3. Fit profile",
                "skyrmion.fit_size(method='auto')",
                "ansatz, domain wall, Gaussian, AICc diagnostics",
            ),
            (
                "4. Combined",
                "skyrmion.analyze()",
                "consistent topology and size result",
            ),
        ]
        sk_rows = "".join(
            f"<tr><td style='padding:4px 8px;color:#67e8f9;font-weight:600;'>{_esc(step)}</td>"
            f"<td style='padding:4px 8px;font-family:monospace;color:#93c5fd;font-size:0.85em;'>{_esc(call)}</td>"
            f"<td style='padding:4px 8px;color:#cbd5e1;'>{_esc(description)}</td></tr>"
            for step, call, description in skyrmion_workflow
        )
        sk_section = (
            f"<div style='{_HELPER_SECTION_CHROME}'>"
            "<b style='color:#94a3b8;'>Skyrmion Workflow</b><br>"
            "<table style='width:100%;border-collapse:collapse;font-size:0.9em;margin-top:6px;'>"
            "<thead><tr style='text-align:left;background:rgba(51,65,85,0.6);'>"
            "<th style='padding:4px 8px;color:#e2e8f0;'>Step</th>"
            "<th style='padding:4px 8px;color:#e2e8f0;'>Access</th>"
        ...
        return node_card_html(
            ...
            icon="🌀",
            subtitle="Vortex dynamics plus skyrmion topology and physical size",
            sections=[status, ns_section, wf_section, sk_section, examples],
            api=api_card,
