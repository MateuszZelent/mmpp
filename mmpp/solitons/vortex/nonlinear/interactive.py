...
class ThieleInteractiveDashboard:
    ...
    def show(self, figsize=(14, 4.5), dpi=100):
        ...
        self.w_alpha = widgets.FloatLogSlider(
            description="Damping α:", value=0.01, base=10, min=-4, max=-1, step=0.01, 
            continuous_update=False, style=style, layout=widgets.Layout(width='95%')
        )
        ...
        # --- TAB 2: STT & Readout ---
        self.w_current = slider("Current I_dc", 6.0, -30.0, 30.0, 0.1, "mA", "DC current injected into the system")
        self.w_pol = slider("Spin Polarization P", 0.3, 0.0, 1.0, 0.02)
        
        # CPP specific
        self.w_lambda = slider("Slonczewski Λ (CPP)", 1.0, 0.5, 5.0, 0.05, "", "Asymmetry parameter for MTJ STT")
        self.w_pz = slider("Polarizer P_z (CPP)", 1.0, -1.0, 1.0, 0.05, "", "Out-of-plane component (excites vortex)")
        
        # CIP specific
        self.w_beta = slider("Non-adiabatic β (CIP)", 0.02, 0.0, 0.2, 0.005, "", "Zhang-Li non-adiabatic parameter")
        self.w_cip_angle = slider("Current Angle (CIP)", 0.0, -180.0, 180.0, 5.0, "deg", "Direction of current in the plane")
        ...
        tab_stt = widgets.VBox([
            ...
            self.w_current, self.w_pol, 
            self.w_lambda, self.w_pz,  # Hideable CPP params
            self.w_beta, self.w_cip_angle, # Hideable CIP params
            self.w_p, self.w_c,
        ...
        self.w_oersted = slider("Oersted Shift (CPP)", -15.0, -50.0, 50.0, 0.5, "MHz per 10 MA/cm²")
        ...
        tab_field = widgets.VBox([
            widgets.HTML("<b>External Magnetic Field</b><br/><i>In-plane fields (Bx, By) shift the core equilibrium based on 's_eq' parameter. OOP field (Bz) shifts frequency based on 'dω₀/dBz'.</i>"),
        ...
        self.w_auto_w0 = widgets.Checkbox(description="Auto-calculate ω₀", value=True, style=style)
        self.w_omega0 = slider("Override Base f₀", 0.395, 0.05, 5.0, 0.01, "GHz")
        self.w_n = slider("Nonlinearity N (CPP)", 0.25, -1.0, 2.0, 0.05, "", "Frequency blue-shift/red-shift coefficient")
        ...
        self.w_seq_per_T = slider("IP Shift s_eq", 1.5, -5.0, 5.0, 0.05, "1/T", "Equilibrium shift per 1T of in-plane field")

        tab_model = widgets.VBox([
            widgets.HTML("<b>Gyrotropic Mode Tuning</b>"),
            self.w_auto_w0, self.w_omega0,
            widgets.HTML("<hr><b>Phenomenological Coupling</b><br/><i>Must be != 0 for fields to have an effect. Fit these to mumax3!</i>"),
        ...
        self.w_sde = widgets.Checkbox(description="Enable Thermal Noise (CPP SDE)", value=False, style=style)
        ...
        tab_solver = widgets.VBox([
            widgets.HTML("<b>Integration Control</b><br/><i>SDE is supported only in the CPP model.</i>"),
        ...
        return widgets.VBox([
            widgets.HTML("<h2 style='color:#334155; margin-bottom:0;'>🌪️ Vortex Dynamics Dashboard</h2>"),
            self.w_model_type,
    ...
    def _update_hud(self, *_args):
        ...
        try:
            ...
            peff = slonczewski_mtj_efficiency(self.w_pol.value, self.w_lambda.value, self.w_pz.value) if is_cpp else self.w_pol.value
            
            current_density_A_m2 = (self.w_current.value * 1e-3) / area
            J_MA_cm2 = current_density_A_m2 / 1e10

            mat = MaterialParams(Ms=self.w_ms.value * 1e3, alpha=self.w_alpha.value, P=peff, beta_nonadiabatic=self.w_beta.value if not is_cpp else None)
            ...
            if is_cpp:
                ...
                status_text = "AUTO-OSCILLATION (CPP)" if is_osc else "DAMPED (Sub-threshold)"
    ...
    def _wire_events(self):
        for w in [self.w_model_type, self.w_geom_mode, self.w_field_mode, self.w_sde, self.w_auto_w0, self.w_rand_seed]:
            w.observe(self._sync_disabled, names="value")

        widgets_to_hud = [
            self.w_model_type, self.w_geom_mode, self.w_disk_d, self.w_size_x, self.w_size_y, self.w_thick,
            self.w_ms, self.w_alpha, self.w_beta, self.w_current, self.w_pol, self.w_lambda, self.w_pz,
            ...
            try:
                ...
                if is_cpp:
                    ...
                    model = CIPThieleModel(
                        material=mat, geom=geo, omega0=omega0_rad, polarity=self.w_p.value,
                ...
                # ── FULL ODE/SDE MODE ──
                    ...
                    if is_cpp:
                        ...
                        traj = model.simulate(
                            t_span=t_span, r0=(1e-3 * radius_eq, 0.0), 
                            J_func=current_dc(current_density_A_m2),
                            B_func=b_func, dt=dt, **kwargs
                        )
                    ...
                    # FFT / Welch
                    freqs, psd = proxy_psd(signal, dt=dt, method="welch" if (is_cpp and self.w_sde.value) else "fft")
                    ...
                    lim = max(R_nm, np.max(np.abs(x_nm - eq_nm[0])) if len(x_nm) > 0 else 0, np.max(np.abs(y_nm - eq_nm[1])) if len(y_nm) > 0 else 0) * 1.05
...
__all__ = ["ThieleInteractiveDashboard", "build_thiele_dashboard", "proxy_signal_from_trajectory", "proxy_psd"]