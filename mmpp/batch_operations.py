    ...
    def compute_all(self, **kwargs) -> Dict[str, Any]:
        ...
        **kwargs : dict
        ...
        for i, result in enumerate(self.results):
            try:
                log.debug(f"Computing FFT for result {i+1}/{len(self.results)}: {result.path}")
        ...
        log.info(f"Batch FFT computation completed: {successful} successful, {failed} failed")
        return summary
...
class BatchModeAnalyzer:
    ...
    def compute_modes(self, dset: str = "m_z5-8", parallel: bool = True, 
        ...
        **kwargs : dict
        ...
        def compute_single_result(result_info):
            ...
            try:
                log.debug(f"Computing modes for result {i+1}/{len(self.results)}: {result.path}")
        ...
        log.info(f"Batch mode computation completed: {successful} successful, {failed} failed")
    ...
    def analyze_all(self, **kwargs) -> Dict[str, Any]:
        ...
        **kwargs : dict
        ...
        for i, result in enumerate(self.results):
            try:
                log.debug(f"Analyzing modes for result {i+1}/{len(self.results)}: {result.path}")
        ...
        log.info(f"Batch mode analysis completed: {successful} successful, {failed} failed")
        return summary
...
class BatchOperations:
    ...
    def prepare_report(self, spectrum: bool = True, modes: bool = True, 
