...
def _patch_matplotlib_tight_layout_warning() -> None:
    ...
    if getattr(Figure.tight_layout, "_mmpp_tight_layout_patched", False):  # type: ignore[attr-defined]
        return
    ...
    _tight_layout_without_warning._mmpp_tight_layout_patched = True  # type: ignore[attr-defined]
...
def check_dependencies(verbose: bool = True):
    ...
    for module, info in status.items():
        ...
        required_packages = cast(list[str], info["required_packages"])
...
try:
    ...
    def build_thiele_dashboard(  # type: ignore[no-redef]
        analyzer: Any | None = None, **kwargs: Any
    ) -> Any:
