...
class ZarrJobResult:
    ...
    def mpl(self) -> 'MMPPlotter':
        ...
        if not PLOTTING_AVAILABLE:
            raise ImportError("Plotting functionality not available. Check plotting.py import.")
        if self._mmpp_ref is None:
            raise ValueError("MMPP reference not set. Use results from MMPP.find() method.")
...
class MMPP:
    ...
    def __init__(self, base_path: str, max_workers: int = 8, database_name: str = "mmpy_database") -> None:
        """
        Initialize the MMPP.
    ...
    def _parse_path_parameters(self, zarr_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for part in path_parts:
                ...
                if not part or (part.startswith('v') and part[1:].isdigit() and len(part) <= 3):
                    continue
    ...
    def _parse_single_path_component(self, component: str) -> Dict[str, Any]:
        ...
        try:
            ...
            match = re.match(r'^(.+)_values_([-+]?[0-9]*\.?[0-9]+(?:[eE][-+]?[0-9]+)?)$', component)
            if match:
            ...
            match = re.match(r'^(.+?)_([-+]?[0-9]*\.?[0-9]+(?:[eE][-+]?[0-9]+)?)$', component)
            if match:
                param_name, param_value = match.groups()
                try:
    ...
    def _scan_all_zarr_folders(self, zarr_folders: List[str]) -> List[ScanResult]:
        ...
        print(f"Scanning {len(zarr_folders)} zarr folders using {self.max_workers} threads...")
    ...
    def find(self, **kwargs: Any) -> 'PlotterProxy':
        ...
        **kwargs : Any
        ...
        for _, row in matching_rows.iterrows():
            path = row['path']
            attributes = {col: row[col] for col in self.dataframe.columns if col != 'path' and pd.notna(row[col])}
    ...
    def find_by_path_param(self, **kwargs: Any) -> List[ZarrJobResult]:
        ...
        **kwargs : Any
        ...
        for key, value in kwargs.items():
            ...
            if not param_found:
                print(f"Warning: Parameter '{key}' not found in database (neither as '{key}' nor '{key}_path')")
                continue
    ...
    def get_path_parameter_summary(self) -> Dict[str, List[Any]]:
        ...
        for col in self.dataframe.columns:
            if col.endswith('_path'):
                ...
                if col not in [c[:-5] for c in self.dataframe.columns if c.endswith('_path')]:
                    unique_vals = sorted(self.dataframe[col].dropna().unique().tolist())
    ...
    def list_data(self, limit: int = 10) -> None:
        ...
        # Show sample data
        display_count = len(self.dataframe) if limit == -1 else min(limit, len(self.dataframe))
    ...
    def _rich_display(self) -> str:
        ...
        for param, values in list(param_summary.items())[:5]:  # Show first 5 parameters
            ...
            param_text.append(f"({min(values)} - {max(values)})\n" if values and isinstance(values[0], (int, float)) else f"({', '.join(map(str, values[:3]))}{'...' if len(values) > 3 else ''})\n", style="dim")
        
        if RICH_AVAILABLE:
            with self.console.capture() as capture:
                self.console.print(Panel.fit(summary_text, title="[bold blue]MMPP Database[/bold blue]", border_style="blue"))
                self.console.print("")
                self.console.print(Columns([
                    Panel.fit(methods_text, title="[bold yellow]Methods[/bold yellow]", border_style="yellow"),
                    Panel.fit(param_text, title="[bold green]Parameters[/bold green]", border_style="green")
                ]))
            return capture.get()
...
"""
        ...
        if ITABLES_AVAILABLE and IPYTHON_AVAILABLE:
            try:
                ...
                for col in df_formatted.columns:
                    if col != 'path' and df_formatted[col].dtype in ['float64', 'float32']:
                        # Check if column has large numbers
                        if df_formatted[col].abs().max() > 1000:
                            df_formatted[col] = df_formatted[col].apply(lambda x: f"{x:.2e}" if pd.notna(x) else "N/A")
                
                # Shorten paths for better display
                if 'path' in df_formatted.columns:
                    df_formatted['path'] = df_formatted['path'].apply(
                        lambda x: str(x).replace(self.base_path, "...") if pd.notna(x) else "N/A"
                    )
    ...
    def _show_rich_table(self, df: pd.DataFrame, max_rows: int = 50) -> None:
        ...
        table = Table(title="MMPP Database", show_header=True, header_style="bold magenta")
        ...
        if len(df) > max_rows:
            table.add_row(*[f"[dim]... and {len(df) - max_rows} more rows[/dim]"] + [""] * (len(df.columns) - 1))
