...
def _display_cluster_info(cluster_data: dict) -> None:
    ...
    if RICH_AVAILABLE:
        ...
        nodes_table.add_row("Free", str(free_nodes), f"{(free_nodes/total_nodes*100):.1f}%" if total_nodes > 0 else "0%")
        nodes_table.add_row("Busy", str(busy_nodes), f"{(busy_nodes/total_nodes*100):.1f}%" if total_nodes > 0 else "0%")
        nodes_table.add_row("Sleeping", str(sleeping_nodes), f"{(sleeping_nodes/total_nodes*100):.1f}%" if total_nodes > 0 else "0%")
        ...
        # Add GPU status rows
        gpus_table.add_row("Free", str(free_gpus), f"{(free_gpus/total_gpus*100):.1f}%" if total_gpus > 0 else "0%")
        gpus_table.add_row("Busy", str(busy_gpus), f"{(busy_gpus/total_gpus*100):.1f}%" if total_gpus > 0 else "0%")
        gpus_table.add_row("Active", str(active_gpus), f"{(active_gpus/total_gpus*100):.1f}%" if total_gpus > 0 else "0%")
        gpus_table.add_row("Standby", str(standby_gpus), f"{(standby_gpus/total_gpus*100):.1f}%" if total_gpus > 0 else "0%")
        gpus_table.add_row("Total", str(total_gpus), "100%")
    else:
        nodes_table_data = [
            "🖥️ Cluster Nodes Summary:",
            f"  Free: {free_nodes} ({(free_nodes/total_nodes*100):.1f}%)" if total_nodes > 0 else f"  Free: {free_nodes} (0%)",
            f"  Busy: {busy_nodes} ({(busy_nodes/total_nodes*100):.1f}%)" if total_nodes > 0 else f"  Busy: {busy_nodes} (0%)",
            f"  Sleeping: {sleeping_nodes} ({(sleeping_nodes/total_nodes*100):.1f}%)" if total_nodes > 0 else f"  Sleeping: {sleeping_nodes} (0%)",
            f"  Total: {total_nodes}"
        ]
        
        gpus_table_data = [
            "🔥 GPU Resources Summary:",
            f"  Free: {free_gpus} ({(free_gpus/total_gpus*100):.1f}%)" if total_gpus > 0 else f"  Free: {free_gpus} (0%)",
            f"  Busy: {busy_gpus} ({(busy_gpus/total_gpus*100):.1f}%)" if total_gpus > 0 else f"  Busy: {busy_gpus} (0%)",
            f"  Active: {active_gpus} ({(active_gpus/total_gpus*100):.1f}%)" if total_gpus > 0 else f"  Active: {active_gpus} (0%)",
            f"  Standby: {standby_gpus} ({(standby_gpus/total_gpus*100):.1f}%)" if total_gpus > 0 else f"  Standby: {standby_gpus} (0%)",
            f"  Total: {total_gpus}"
        ]
