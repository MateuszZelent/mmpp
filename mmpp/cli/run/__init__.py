...
from .submit import (
    handle_run_command as handle_file_submission,
)
...
def _handle_batch_submission(files: list[str], args: argparse.Namespace) -> None:
    ...
    response = (
        input(f"Submit all {len(files)} simulations to queue? [y/N]: ").strip().lower()
    )
        ...
        try:
            ...
            if task_id:
                submitted_tasks.append(
                    {"task_id": task_id, "file": file, "name": os.path.basename(file)}
                )
    ...
    if submitted_tasks:
        ...
        if not getattr(args, "detach", False):
            response = (
                input(
                    f"\nMonitor progress of all {len(submitted_tasks)} simulations? [y/N]: "
                )
                .strip()
                .lower()
            )
                    ...
                    if is_completed:
                        ...
                        if status == "COMPLETED":
                            ...
                            success, zarr_path, error = downloader.download_results(
                                task_id, original_file
                            )

                            if success and zarr_path:
                                print(
                                    f"     ✅ Downloaded: {os.path.basename(zarr_path)}"
                                )
                            else:
