...
class ResultDownloader:
    ...
    def download_results(
        self, task_id: str, original_file_path: str
        ...
        try:
            ...
            # Extract and process
            zarr_file = self._extract_and_process(
                zip_path, output_dir, original_file.stem
            )
            ...
            if zarr_file:
                logger.info(
                    f"Results successfully downloaded and extracted: {zarr_file}"
                )
    ...
    def _download_zip(self, task_id: str, output_dir: Path) -> Path | None:
    ...
    def _extract_and_process(
        self, zip_path: Path, output_dir: Path, base_name: str
    ) -> Path | None:
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as zip_ref:
                ...
                if metadata_files:
                    logger.info("Metadata files included in ZARR directory")
    ...
    def check_task_completion(self, task_id: str) -> tuple[bool, str, dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to check task status: HTTP {response.status_code}"
                )
                return False, "ERROR", {}
