...
def _submit_single_file(args: argparse.Namespace) -> str | None:
    ...
    try:
        ...
        task_id = _upload_and_start_simulation(file_path, base_url, headers, args)
...
def handle_run_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if getattr(args, "detach", False):
            ...
            print(
                "🔄 Running in detached mode. Use 'mmpp run check' to check status and download results."
            )
        else:
...
def handle_check_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        for sim in all_sims:
            ...
            if zarr_exists:
                print(
                    f"   📦 Results: Already downloaded ({os.path.basename(zarr_path)})"
                )
                continue

            # Check status on server
            try:
                is_completed, status, task_info = downloader.check_task_completion(
                    sim.task_id
                )
                ...
                if is_completed and status == "COMPLETED":
                    ...
                    # Download results
                    success, downloaded_zarr_path, error = downloader.download_results(
                        sim.task_id, sim.file_path
                    )
                    ...
                    if success and downloaded_zarr_path:
                        ...
                        print(
                            f"   ✅ Results downloaded: {os.path.basename(downloaded_zarr_path)}"
                        )
                        downloaded_count += 1
                    else:
...
def _upload_and_start_simulation(
    file_path: str, base_url: str, headers: dict, args: argparse.Namespace
) -> str | None:
    ...
    task_name = getattr(args, "name", None) or os.path.basename(file_path).replace(
        ".mx3", ""
    )
        ...
        try:
            ...
            if is_completed:
                if status == "COMPLETED":
                    ...
                    # Download results
                    success, zarr_path, error = downloader.download_results(
                        task_id, original_file_path
                    )
