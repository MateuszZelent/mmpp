...
def list_active_jobs(server_url: str | None = None) -> None:
...
def display_jobs(jobs: list[dict[str, Any]]) -> None:
    ...
    if not jobs:
        if RICH_AVAILABLE:
            console.print("📭 [yellow]No active jobs found[/yellow]")  # type: ignore[union-attr]
        else:
...
def display_jobs_rich(jobs: list[dict[str, Any]]) -> None:
    ...
    console.print(f"\n[dim]{summary_text}[/dim]")  # type: ignore[union-attr]

    # Show monitoring info if available
    monitored = len([j for j in jobs if j.get("monitoring_active")])
    if monitored > 0:
        console.print(f"[dim]📈 Monitoring active: {monitored} jobs[/dim]")  # type: ignore[union-attr]
...
def display_jobs_simple(jobs: list[dict[str, Any]]) -> None:
    ...
    # Header
    print(
        f"{'Job ID':<10} {'Name':<20} {'User':<10} {'State':<10} {'Node':<8} {'Time Used':<12} {'Memory':<8}"
    )
    print("-" * 80)

    for job in jobs:
        print(
            f"{str(job.get('job_id', 'N/A')):<10} "
