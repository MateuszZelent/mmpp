...
def submit_python_code(
    ...
    **kwargs,
    ...
    if full_name is False:
        ...
        sim_name_params = par_sep.join(
            [
                f"{key}{val_sep}{format(val, '.5g')}"
                for key, val in kwargs.items()
                if key != "prefix" and key != "i"
            ]
        )
    ...
    if os.path.isdir(f"{path}.zarr/end") and force is not True:
        ...
        if sbatch is True:
            ...
            with open(sim_sbatch_path, "w") as f:
                f.write(
                    gen_sbatch_script(
                        sim_name, path + sim_name, main_path, destination_path
                    )
                )
...
destination_path = (
    "/mnt/Primary/zfn/mateuszz/mannga/jobs/mateuszz/squircle/FMR/coupled_resonator/"
)
