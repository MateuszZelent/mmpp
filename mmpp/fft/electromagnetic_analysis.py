...
class PoyntingVectorAnalysis:
    ...
    def compute_poynting_vector(self, E_field: np.ndarray, H_field: np.ndarray) -> np.ndarray:
        """
    ...
    def compute_energy_density(self, E_field: np.ndarray, H_field: np.ndarray, 
                              epsilon_r: float = 1.0, mu_r: float = 1.0) -> Dict[str, np.ndarray]:
        """
    ...
    def visualize_energy_flow(self, poynting_vector: np.ndarray, energy_density: np.ndarray,
                             extent: Tuple[float, float, float, float] = None,
                             title: str = "Electromagnetic Energy Flow") -> plt.Figure:
        """
        ...
        fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=self.config.figsize, dpi=self.config.dpi)
        
        ny, nx = energy_density.shape
        extent = extent or (0, nx, 0, ny)
        
        # Energy density map
        im1 = ax1.imshow(energy_density, extent=extent, origin='lower', 
                        cmap=self.config.poynting_colormap, aspect='equal')
        ...
        im2 = ax2.imshow(poynting_mag, extent=extent, origin='lower',
                        cmap=self.config.poynting_colormap, aspect='equal')
        ...
        ax3.streamplot(X_sub, Y_sub, Sx_sub, Sy_sub, 
                      color=poynting_mag[::step, ::step], 
                      cmap=self.config.poynting_colormap, density=1.5)
...
class RadiationPatternAnalysis:
    ...
    def compute_far_field(self, current_density: np.ndarray, 
                         spatial_extent: Tuple[float, float, float, float],
                         frequency: float) -> Dict[str, np.ndarray]:
        """
        ...
        for i in range(ny):
            for j in range(nx):
                ...
                E_theta += (theta_hat_x * J_cross_r[0] + 
                           theta_hat_y * J_cross_r[1] + 
                           theta_hat_z * J_cross_r[2]) * phase_factor * dx * dy
                           
                E_phi += (phi_hat_x * J_cross_r[0] + 
                         phi_hat_y * J_cross_r[1] + 
                         phi_hat_z * J_cross_r[2]) * phase_factor * dx * dy
        
        # Apply far-field scaling
        scaling = 1j * k * np.exp(-1j * k * self.config.far_field_distance) / (4 * np.pi * self.config.far_field_distance)
        E_theta *= scaling
        E_phi *= scaling
    ...
    def visualize_radiation_pattern(self, far_field_data: Dict[str, np.ndarray],
                                  title: str = "Radiation Pattern") -> plt.Figure:
        """
        ...
        im = ax3.contourf(Phi, Theta, intensity_db, levels=20, cmap=self.config.radiation_colormap)
...
class QFactorAnalysis:
    ...
    def compute_q_factor_spectral(self, frequencies: np.ndarray, 
                                 spectrum: np.ndarray, peak_freq: float) -> float:
        """
...
def analyze_electromagnetic_properties(mode_data, config: ElectromagneticAnalysisConfig = None) -> Dict[str, Any]:
    """
    ...
    log.info(f"Starting electromagnetic analysis for frequency {mode_data.frequency:.3f} GHz")
    ...
    try:
        ...
        far_field = radiation_analyzer.compute_far_field(current_density, extent_m, omega/(2*np.pi))
...
def create_comprehensive_em_report(mode_data, analysis_results: Dict[str, Any], 
                                  config: ElectromagneticAnalysisConfig = None) -> plt.Figure:
    """
    ...
    im1 = ax1.imshow(mode_magnitude, extent=mode_data.extent, origin='lower', cmap='cmc.berlin')
    ...
    if analysis_results.get('analysis_successful', False):
        ...
        im2 = ax2.imshow(energy_total, extent=mode_data.extent, origin='lower', cmap='cmc.hawaii')
        ...
        im3 = ax3.imshow(poynting_mag, extent=mode_data.extent, origin='lower', cmap='cmc.hawaii')
        ...
        ax4.streamplot(X[::step, ::step], Y[::step, ::step],
                      ...
                      color=poynting_mag[::step, ::step], cmap='cmc.hawaii', density=1.2)
        ...
        intensity_db = 10*np.log10(far_field['radiation_intensity'] / np.max(far_field['radiation_intensity']))
        ...
        total_energy = np.sum(energy_total) * (mode_data.extent[1] - mode_data.extent[0]) * (mode_data.extent[3] - mode_data.extent[2]) * 1e-18  # Convert nm² to m²
        ...
        ax_error.text(0.5, 0.5, error_msg, transform=ax_error.transAxes, 
                     fontsize=16, ha='center', va='center', color='red')
    
    plt.suptitle(f'Comprehensive Electromagnetic Analysis - {mode_data.frequency:.3f} GHz', fontsize=18)
