...
class SpectrumEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if self.frequencies.ndim != 1 or self.frequencies.size == 0:
            raise ValueError("SpectrumEntry frequencies must be a non-empty 1D array")
        if self.spectrum.shape != self.frequencies.shape:
            raise ValueError("SpectrumEntry spectrum must match the frequency axis")
        if self.power.shape != self.frequencies.shape:
            raise ValueError("SpectrumEntry power must match the frequency axis")
        if (
            not np.isfinite(self.frequencies).all()
            or not np.isfinite(self.spectrum).all()
        ):
            raise ValueError("SpectrumEntry frequencies and spectrum must be finite")
        if not np.isfinite(self.power).all() or np.any(self.power < 0):
            raise ValueError("SpectrumEntry power must be finite and non-negative")
        if not isinstance(self.parameters, dict):
            raise TypeError("SpectrumEntry parameters must be a dictionary")
        if not isinstance(self.index, (int, np.integer)) or isinstance(
            self.index, (bool, np.bool_)
        ):
            raise TypeError("SpectrumEntry index must be an integer")
    ...
    def plot(
        ...
        **kwargs
        ...
        param_str = ", ".join(
            f"{k}={v}" for k, v in self.parameters.items() if v is not None
        )
        ax.set_title(
            f"Spectrum [{self.index}]: {param_str}"
            if param_str
...
class BatchSpectrumResult:
    ...
    def __init__(
        ...
        self.spectra = [np.asarray(value) for value in spectra]
        self.powers = [np.asarray(value, dtype=float) for value in powers]
        self.parameters = dict(parameters)
        self.job_paths = [str(path) for path in job_paths]
    ...
    def _validate_integrity(self) -> None:
        ...
        if getattr(self, "schema_version", None) != BATCH_SPECTRUM_SCHEMA_VERSION:
            raise ValueError(
                "Incompatible batch spectrum cache schema; recompute the batch"
            )
        if self.frequencies.ndim != 1 or self.frequencies.size == 0:
            raise ValueError("Batch frequencies must be a non-empty 1D array")
        if not np.isfinite(self.frequencies).all() or np.any(
            np.diff(self.frequencies) <= 0
        ):
            raise ValueError("Batch frequencies must be finite and strictly increasing")
        ...
        for index, (spectrum, power) in enumerate(
            ...
            if spectrum.shape != self.frequencies.shape:
                raise ValueError(
                    f"Spectrum {index} shape must match the frequency axis"
                )
            if power.shape != self.frequencies.shape:
                raise ValueError(f"Power {index} shape must match the frequency axis")
            if not np.isfinite(spectrum).all():
                raise ValueError(f"Spectrum {index} must be finite")
            if not np.isfinite(power).all() or np.any(power < 0):
                raise ValueError(f"Power {index} must be finite and non-negative")
        if not isinstance(self.parameters, dict):
            raise TypeError("parameters must be a dictionary")
        for name, values in self.parameters.items():
            if len(values) != count:
                raise ValueError(
                    f"Parameter {name!r} has {len(values)} values for {count} spectra"
                )
        if not self.dataset_name:
            raise ValueError("dataset_name must be non-empty")
        if not isinstance(self.z_layer, (int, np.integer)) or isinstance(
            self.z_layer, (bool, np.bool_)
        ):
            raise TypeError("z_layer must be an integer")
        self.z_layer = int(self.z_layer)
    ...
    def __getitem__(self, index: int) -> SpectrumEntry:
        ...
        if index < 0 or index >= len(self):
            raise IndexError(
                f"Index {index} out of range for batch of {len(self)} spectra"
            )
    ...
    def show_parameters(self) -> None:
        ...
        """
        print("📊 Batch Spectrum Parameters Summary")
        print(f"{'=' * 60}")
        ...
        if varying:
            ...
            for p in sorted(varying, key=lambda x: x["n_unique"], reverse=True):
                print(
                    f"    • {p['name']}: {p['n_unique']} unique values "
        ...
        if varying:
            ...
            print(
                f"   result.plot_heatmap(parameter='{varying[0]['name']}')  # Explicit"
            )
        else:
            print("   (No varying parameters - cannot create heatmap)")
        print(f"{'=' * 60}\n")

    def _apply_folding(
        self, param_values: np.ndarray, sort_idx: np.ndarray, folding_period: float
    ...
    def load(cls, path: str | Path) -> BatchSpectrumResult:
        ...
        if path.suffix == ".zarr" and ZARR_AVAILABLE:
            ...
            if not isinstance(loaded, cls):
                raise TypeError("Pickle does not contain a BatchSpectrumResult")
            loaded._validate_integrity()
            return loaded
    ...
    def _replicate_experimental_points(
        self, angles: np.ndarray, fres: np.ndarray, fres_err: np.ndarray, folding: float
