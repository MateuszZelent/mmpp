...
class BatchTransmissionResult:
    ...
    def plot_transmission_crosssection_heatmap_difference(
        ...
        **kwargs,
        ...
        freq_b, heatmap_b, _, param_values_b = other._prepare_crosssection_heatmap_data(
            swapping_parameter=swapping_parameter,
            x=x,
            x_width=x_width,
            freq_unit=freq_unit,
        ...
        if len(param_values_a) != len(param_values_b) or not np.allclose(
            ...
            if align_strategy in ("auto", "intersect"):
                idx_a, idx_b, matched_vals = _match_params(
                    param_values_a, param_values_b, atol, rtol
                )
                if len(idx_a) == 0:
                    raise ValueError(
        ...
        if len(freq_a) != len(freq_b) or not np.allclose(
            ...
            if verbose:
                log.info(
                    "Aligned second batch heatmap to %d frequency points",
                    len(freq_target),
                )
    ...
    def _extract_crosssection(
        ...
        if verbose:
            ...
            print(
                f"    cross_section range: [{cross_section.min():.4e}, {cross_section.max():.4e}]"
            )
        ...
        if verbose:
            print(
                f"    final cross_section range: [{cross_section.min():.4e}, {cross_section.max():.4e}]"
            )
            print(
                f"    final freq range: [{freq_displayed.min():.2f}, {freq_displayed.max():.2f}] {freq_unit}"
            )
...
class BatchTransmission:
    ...
    def compute_all(
        ...
        **kwargs
        ...
        if not force and save_batch and batch_cache_file.exists():
            try:
                ...
                if len(cached_result.results) == len(self.results):
                    log.info(
                        f"✅ Loaded {len(cached_result.results)} results from cache"
                    )
        ...
        log.info(
            f"Starting batch transmission computation for {len(self.results)} results"
        )
        ...
        if parallel and len(self.results) > 1:
            ...
            if max_workers is None:
                # Default: limit workers to prevent OOM on large batches
                # Each transmission computation loads significant data into RAM
                max_workers = min(len(self.results), 4)
...
def stack_results(
    ...
    log.info(f"Stacked {len(results)} results into array with shape {stacked.shape}")
