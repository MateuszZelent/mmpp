...
class CacheManager:
    ...
    def load_cached_dispersion(
        ...
        from ..models import DispersionConfig, DispersionResult1D
        ...
        result = DispersionResult1D(
            ...
            S=S,
            S_complex=S_complex,
            axis=str(attrs.get("axis", "x")),
            component=str(attrs.get("component", "perp")),
            config=DispersionConfig(),
            dx=float(attrs.get("dx", 0.0)),
            orth_axis=self._load_group_array(result_group, "orth_axis"),
        )
        result.metadata = attrs.get("metadata", {})  # type: ignore[attr-defined]
    ...
    def _get_external_cache_group(self, write: bool) -> zarr.Group | None:
        ...
        try:
            zarr.open(str(cache_path), mode=mode)
    ...
    def _get_job_cache_group(self, write: bool) -> zarr.Group | None:
        """Get job zarr cache group (<job>.zarr/fft/dispersion/<dataset>/)."""
        mode = "a" if write else "r"
        try:
            zarr.open(self.interface.parent_fft.job_result.path, mode=mode)
        except (OSError, PermissionError, FileNotFoundError) as exc:
            if write:
                raise
