...
def generate_animation_frames(
    ...
    for i, t in enumerate(time_array):
        frames[i] = compute_spinwave_field(amplitude, phase, omega, t, damping_time)
...
class SpinWaveModeAnimator:
    ...
    def plot_snapshot(
        ...
        if show_envelope:
            ax.plot(
                ...
                linewidth=1.5,
                alpha=0.7,
                label=r"$A(y)$ - envelope",
            )
            ax.plot(y_um, -self.amplitude, "r--", linewidth=1.5, alpha=0.7)
            ax.fill_between(
                y_um, -self.amplitude, self.amplitude, alpha=0.1, color="red"
            )
    ...
    def animate(
        ...
        def update(frame_idx):
            ...
            if frames is None or time_array is None:
                raise RuntimeError("Animation frames have not been generated")
            m_field = frames[frame_idx]
            ...
            time_text.set_text(f"t = {t_ns:.2f} ns\nphase = {phase_cycles:.2f} cycles")
        ...
        self._anim = anim  # type: ignore[assignment]
