...
def _resolve_loop_axis_labels(result) -> tuple[str, str]:
    ...
    if isinstance(column, str) and column.strip():
        ...
        if c in {"mx", "my", "mz"}:
            ylabel = rf"$m_{c[-1]}$"
        elif c in {"m_full", "norm", "|m|", "magnitude"}:
            ylabel = r"$|m|$"
        else:
            ylabel = column.strip()
    else:
...
def create_animation(
    ...
    *,
    ...
    # Local import avoids circular dependency during module import:
    # animation -> plot._interactive.callbacks -> animation.
    ...
    component = str(snapshot_component if snapshot_component is not None else result.config.snapshot_component)
    ...
    if result.branches:
        for branch in result.branches:
            ...
            if show_branch_colors_val:
                color = result.config.branch_colors[0] if branch.name == "ascending" else result.config.branch_colors[1]
            else:
                color = "#3b82f6"
    ...
    if show_hc_val:
        ...
        for h in (hc.hc_minus, hc.hc_plus):
            if np.isfinite(h):
                ax_loop.axvline(float(h), ls="--", lw=1.1, color="#f97316", alpha=0.75)
    if show_mr_val:
        mr = result.metrics.remanence
        for m in (mr.mr_minus, mr.mr_plus):
            if np.isfinite(m):
                ax_loop.axhline(float(m), ls=":", lw=1.1, color="#a855f7", alpha=0.75)
    if show_ms_val:
