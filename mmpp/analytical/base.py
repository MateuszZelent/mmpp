...
class DispersionResult(AnalyticalResult):
    ...
    def __repr__(self) -> str:
        k_range = f"[{self.k.min():.2e}, {self.k.max():.2e}]" if len(self.k) > 0 else "[]"
        f_range = f"[{self.f.min():.2f}, {self.f.max():.2f}]" if len(self.f) > 0 else "[]"
        return (f"DispersionResult(model={self.model_name!r}, "
...
class FMRResult(AnalyticalResult):
    ...
    def __repr__(self) -> str:
        B_range = f"[{self.B.min():.3f}, {self.B.max():.3f}]" if len(self.B) > 0 else "[]"
        f_range = f"[{self.f.min():.2f}, {self.f.max():.2f}]" if len(self.f) > 0 else "[]"
        return (f"FMRResult(model={self.model_name!r}, "
...
class PlotAccessor:
    ...
    def fill_between(
        self,
        other: Union[AnalyticalResult, np.ndarray, float],
        ax: Optional["Axes"] = None,
        **kwargs,
    ) -> Any:
        """
