...
try:
    ...
    for i, result in enumerate(mock_results):
        ...
        print(f"    Can access solver? {hasattr(result, 'solver')} -> {getattr(result, 'solver', 'N/A')}")
        print(f"    Can access f0? {hasattr(result, 'f0')} -> {getattr(result, 'f0', 'N/A')}")
    ...
    for i, result in enumerate(mock_results):
        print(f"  Result {i+1}:")
        print(f"    result.attributes['solver']: {result.attributes.get('solver', 'MISSING')}")
        ...
        print(f"    test_get_value(result, 'solver'): {test_get_value(result, 'solver')}")
