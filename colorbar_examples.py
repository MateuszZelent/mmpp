...
def enhanced_mode_plot_example():
    ...
    mode_data = np.sin(2*np.pi*X/25) * np.cos(2*np.pi*Y/25) * np.exp(-((X-50)**2 + (Y-50)**2)/1000)
    
    # Create figure
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    
    # === Standard colorbar (existing MMPP style) ===
    im1 = ax1.imshow(mode_data, 
                     cmap='RdBu_r',
                     extent=[0, 100, 0, 100],
                     aspect='equal',
                     origin='lower')
...
def colormap_comparison():
    ...
    mode_data = np.sin(2*np.pi*X/25) * np.cos(2*np.pi*Y/25) * np.exp(-(X**2 + Y**2)/1000)
...
def dark_theme_example():
    ...
    ax.set_title('Dark Theme Optimized Mode Visualization', 
