    ...
    for i in range(n_x):
        for j in range(n_y):
            for k in range(n_z):
                ...
                m_data[:, k, j, i, 0] = 0.1 * np.sin(phase1) + 0.05 * np.cos(phase2)  # mx
                m_data[:, k, j, i, 1] = 0.1 * np.cos(phase1) + 0.05 * np.sin(phase2)  # my  
                m_data[:, k, j, i, 2] = 0.9 + 0.05 * np.sin(phase1) + 0.02 * np.cos(phase2)  # mz
...
def test_fft_save_functionality():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if np.allclose(result1.frequencies, result2.frequencies) and \
           np.allclose(result1.spectrum, result2.spectrum):
        ...
        if not np.allclose(result1.spectrum, result3.spectrum):
            print("✓ Parameter verification working - different parameters produce different results")
        else:
        ...
        if np.allclose(result1.frequencies, result4.frequencies, rtol=1e-10) and \
           np.allclose(result1.spectrum, result4.spectrum, rtol=1e-10):
        ...
        if 'fft' in z:
            ...
            for dataset_name in z['fft'].keys():
                ...
                expected_attrs = ['window_function', 'filter_type', 'fft_engine', 'zero_padding']
...
def test_high_level_interface():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"  - Auto-generated dataset name: {result.metadata.get('save_dataset_name', 'N/A')}")
